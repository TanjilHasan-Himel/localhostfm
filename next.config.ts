...
const securityHeaders = [
  ...
  // Content Security Policy — the main shield against XSS, data injection, prompt injection via content
  {
    ...
    value: [
      ...
      "media-src 'self' blob: https://stream.zeno.fm http://localhost:8000 https://*.trycloudflare.com https://*.somafm.com http://listen.181fm.com",
      // Allow connections to our own API routes only
      "connect-src 'self' https://stream.zeno.fm http://localhost:8000 https://*.trycloudflare.com https://*.somafm.com http://listen.181fm.com",
